...
class NeuralNetwork:
    def __init__(self,x,y):
        ...
        self.layer1W = np.random.rand(self.input.shape[1],2)
        self.layer2W = np.random.rand(2,5) 
        self.layer3W = np.random.rand(5,3)
        self.layer4W = np.random.rand(3,1)
    ...
    def feedforward(self, test = None):
        if test is None:
            self.layer1R = mySigmoid(np.dot(self.input, self.layer1W))
        else:
            self.layer1R = mySigmoid(np.dot(test.values, self.layer1W))
            
        self.layer2R = mySigmoid(np.dot(self.layer1R, self.layer2W))
        self.layer3R = mySigmoid(np.dot(self.layer2R, self.layer3W))
        self.output = mySigmoid(np.dot(self.layer3R, self.layer4W))
        
    def backprop(self):
        epsilon = 0.3
        
        delta_W41 = epsilon * (1 - self.output) * (self.y - self.output) * self.output * self.layer3R[:, 0].reshape(-1,1)
        delta_W42 = epsilon * (1 - self.output) * (self.y - self.output) * self.output * self.layer3R[:, 1].reshape(-1,1)
        delta_W43 = epsilon * (1 - self.output) * (self.y - self.output) * self.output * self.layer3R[:, 2].reshape(-1,1)
        ...
        top_error = (1 - self.output) * (self.y - self.output) * self.output
        ...
        delta_W31 = epsilon * (1 - self.layer3R[:,0].reshape(-1,1)) * ec31 * self.layer3R[:,0].reshape(-1,1) * self.layer2R
        
        # All connections to neuron 3,2
        delta_W32 = epsilon * (1 - self.layer3R[:,1].reshape(-1,1)) * ec32 * self.layer3R[:,1].reshape(-1,1) * self.layer2R
        
        delta_W33 = epsilon * (1 - self.layer3R[:,2].reshape(-1,1)) * ec32 * self.layer3R[:,2].reshape(-1,1) * self.layer2R
        ...
        errorL31 = (1 - self.layer3R[:,0].reshape(-1,1)) * ec31 * self.layer3R[:,0].reshape(-1,1)
        errorL32 = (1 - self.layer3R[:,1].reshape(-1,1)) * ec32 * self.layer3R[:,1].reshape(-1,1)
        errorL33 = (1 - self.layer3R[:,2].reshape(-1,1)) * ec33 * self.layer3R[:,1].reshape(-1,1)
        ...
        delta_W21 = epsilon * (1 - self.layer2R[:,0].reshape(-1,1)) * ec21 * self.layer2R[:,0].reshape(-1,1) * self.layer1R
        delta_W22 = epsilon * (1 - self.layer2R[:,1].reshape(-1,1)) * ec22 * self.layer2R[:,1].reshape(-1,1) * self.layer1R
        delta_W23 = epsilon * (1 - self.layer2R[:,2].reshape(-1,1)) * ec23 * self.layer2R[:,2].reshape(-1,1) * self.layer1R
        delta_W24 = epsilon * (1 - self.layer2R[:,3].reshape(-1,1)) * ec24 * self.layer2R[:,3].reshape(-1,1) * self.layer1R
        delta_W25 = epsilon * (1 - self.layer2R[:,4].reshape(-1,1)) * ec25 * self.layer2R[:,4].reshape(-1,1) * self.layer1R
        ...
        errorL21 = (1 - self.layer2R[:,0].reshape(-1,1)) * ec21 * self.layer2R[:,0].reshape(-1,1)
        errorL22 = (1 - self.layer2R[:,1].reshape(-1,1)) * ec22 * self.layer2R[:,1].reshape(-1,1)
        errorL23 = (1 - self.layer2R[:,2].reshape(-1,1)) * ec23 * self.layer2R[:,2].reshape(-1,1)
        errorL24 = (1 - self.layer2R[:,3].reshape(-1,1)) * ec24 * self.layer2R[:,3].reshape(-1,1)
        errorL25 = (1 - self.layer2R[:,4].reshape(-1,1)) * ec25 * self.layer2R[:,4].reshape(-1,1)
        ...
        delta_W11 = epsilon * (1 - self.layer1R[:,0].reshape(-1,1)) * ec11 * self.layer1R[:,0].reshape(-1,1) * self.input
        delta_W12 = epsilon * (1 - self.layer1R[:,1].reshape(-1,1)) * ec11 * self.layer1R[:,1].reshape(-1,1) * self.input
...
# # Initialize Neural Network with training data and train by feedforward and backprop 1500 times (adjustable)
# nn = NeuralNetwork(X, Y)
# for i in range(1500):
#     nn.feedforward()
#     nn.backprop()

# # Run neural net, fit to train data, with test data, output score
# nn.feedforward(xx)
# p = nn.output
# count = 0
...
# # Importing Neural Network package from sklearn from python
...
# # Re initializing data to fit package requirments
# X = train[['Age','Workclass','Education','Marital Status','Occupation','Relationship','Race','Sex','Capital Gain',
#                'Capital Loss','Hours per Week','GDP per Cap','Constant']]
# Y = train['Salary'].values

# # Import Cross validation package and observe folds accuracy 
